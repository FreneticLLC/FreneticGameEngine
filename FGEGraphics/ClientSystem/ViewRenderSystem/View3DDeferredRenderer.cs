...
public class View3DDeferredRenderer : View3DCoreDataSet
{
    ...
    public void RenderPass_Lights()
    {
        ...
        if (Engine.Deferred_Shadows)
        {
            ...
            GL.Uniform1(ShaderLocations.LightAdder.DEPTH_JUMP, Config.ShadowBlur);
        }
        else
        {
            if (Engine.Deferred_SSAO)
        ...
        GL.Uniform3(ShaderLocations.LightAdder.AMBIENT, Config.Ambient.ToOpenTK());
        ...
        lights_apply:
            GL.ActiveTexture(TextureUnit.Texture4);
            GL.BindTexture(TextureTarget.Texture2DArray, Internal.FBO_Shadow_DepthTexture);
            GL.Uniform2(ShaderLocations.LightAdder.Z_DISTANCE, new Vector2(Engine.ZNear, Engine.ZFar()));
            GL.UniformMatrix4(ShaderLocations.LightAdder.SSAO_PROJECTION, false, ref State.PrimaryMatrix); // TODO: In 3D/VR, render both eyes separately here for SSAO accuracy?
            GL.Uniform1(ShaderLocations.LightAdder.LIGHTS_USED, (float)c);
            GL.UniformMatrix4(ShaderLocations.LightAdder.SHADOW_MATRIX_ARRAY, View3DInternalData.LIGHTS_MAX, false, shadowmat_dat);
    ...
    public void RenderPass_HDR()
    {
        if (Engine.Deferred_Lights && Engine.Deferred_DynamicExposure)
        {
            ...
            GL.UniformMatrix4(ShaderLocations.Common.PROJECTION, false, ref View3DInternalData.SimpleOrthoMatrix);
            GL.UniformMatrix4(ShaderLocations.Common.WORLD, false, ref View3DInternalData.IdentityMatrix);
            GL.Uniform2(ShaderLocations.HDRPass.SCREEN_SIZE, new Vector2(Config.Width, Config.Height));
            Engine.Rendering.RenderRectangle(-1, -1, 1, 1);
            View3D.StandardBlend();
    ...
    public void RenderPass_Bloom(int lightc)
    {
        ...
        if (Config.GodRays)
        {
            ...
            GL.UniformMatrix4(ShaderLocations.Common.PROJECTION, false, ref View3DInternalData.SimpleOrthoMatrix);
            GL.UniformMatrix4(ShaderLocations.Common.WORLD, false, ref View3DInternalData.IdentityMatrix);
            GL.Uniform1(ShaderLocations.Godray.EXPOSURE, State.CurrentExposure * Engine.Exposure);
            GL.Uniform1(ShaderLocations.Godray.ASPECT_RATIO, Config.Width / (float)Config.Height);
            if (Config.SunLocation.IsNaN())
            {
                GL.Uniform2(ShaderLocations.Godray.SUN_LOCATION, new Vector2(-10f, -10f));
            }
            else
            {
                ...
                else
                {
                    ...
                    GL.Uniform2(ShaderLocations.Godray.SUN_LOCATION, new Vector2((float)lp1.X, (float)lp1.Y));
                    float lplenadj = (float)((1.0 - Math.Min(lp1.Length, 1.0)) * (0.99 - 0.6) + 0.6);
                    GL.Uniform1(ShaderLocations.Godray.DENSITY, 0.84f * lplenadj);
                }
            }
            GL.Uniform1(ShaderLocations.Godray.MIN_DEPTH, Engine.ZNear);
            GL.Uniform1(ShaderLocations.Godray.MAX_DEPTH, Engine.ZFar());
            GL.Uniform1(ShaderLocations.Godray.SKY_DISTANCE, Engine.ZFarOut() * 0.5f);
            View3D.TranspBlend();
            Engine.Rendering.RenderRectangle(-1, -1, 1, 1);
            View3D.StandardBlend();
        }
        GraphicsUtil.CheckError("PostGR");
        {
            ...
            GL.UniformMatrix4(ShaderLocations.Common.PROJECTION, false, ref View3DInternalData.SimpleOrthoMatrix);
            GL.UniformMatrix4(ShaderLocations.Common.WORLD, false, ref View3DInternalData.IdentityMatrix);
            GL.Uniform1(ShaderLocations.TranspAdder.LIGHTS_USED, (float)lightc);
            Engine.Rendering.RenderRectangle(-1, -1, 1, 1);
        }
        GL.UseProgram(0);
        GL.BindTexture(TextureTarget.Texture2D, 0);
